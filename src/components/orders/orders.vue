<template>
  <style>
    ...
    <swiper :index.sync="index" :threshold="150" :show-dots="false" @on-index-change="getList(index, '', 'click')">
      <swiper-item>
        <card v-link="{ path: '/orders/' + el.id }" style="font-size: 12px;" v-for="el in dataList" :header="{title: '订单号：'+el.order_no}">
          <flexbox slot="content" class="card-padding">
            <flexbox-item>
              <div>
                <span class="fa fa-user list-icon"></span>
                {{el.name}}
              </div>
            ...
            <x-button mini type="primary" v-link="{ path: '/orders/' + el.id }" style="font-size: 14px;position: relative;left: -30px;">查看</x-button>
          </flexbox>
        </card>
        <box gap="10px 10px">
      ...
      this.searchTimer = setTimeout(() => {
        ...
        setTimeout(() => {
          if (url == undefined || url == '') {
            this.$http.get(this.$http.options.root+'/api/shop/orders',{
              params: {
                "status": status,
                "keyword": _this.key
              }
